...
const generateProjectAnalysis = async ({project_desc, start_date, end_date}) => {
  ...
  const [productTypes] = await projectModel.getAllPlatform()
  const [roles] = await projectModel.getAllPlatform()
  const [languages] = await projectModel.getAllPlatform()
  const [tools] = await projectModel.getAllPlatform()
